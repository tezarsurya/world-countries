    ...
    <Menu.Item>
      {({ active }) => (
        <button
          ...
          } w-full rounded-md px-4 py-1 text-left font-semibold dark:text-white/90`}
        >
          {text}
        </button>
      )}
...
const FilterDropdown = () => {
  ...
  return (
    <div className='relative z-10 mt-8 w-full md:mt-0 md:w-fit'>
      <Menu>
        <Menu.Button className='flex w-[60%] items-center justify-between space-x-1 rounded-md bg-white px-6 py-4 shadow-lg dark:bg-[#2b3945] dark:text-white/90 md:w-48 md:py-3'>
          <span>{region}</span>
